...
-- Textclock
local mytextclock = wibox.widget.textclock(markup(white, " %A, %B %d  [%H:%M] "))
...
local special_tags = {"Home", "Browser", "Chat", "Dev", "Media", "Misc", "Game", "Steam"}
local tag_contexts = {}

function theme.set_wallpaper(s)
        local tag = awful.screen.focused().selected_tag
        local context = nil
        --naughty.notify({preset = naughty.config.presets.critical, text = tag, title = "TAG" })
        if (tag) then
                --theme.wallpaper = theme.wallpaperDir .. tag.name .. ".png"
                context = tag_contexts[tag.name]
        end
        ...
        gears.wallpaper.maximized(context, s, true)
end
...
function theme.at_screen_connect(s)
        ...
        for _,name in ipairs(special_tags) do
                local t = awful.tag.find_by_name(s, name)
                if (t) then
                        surface = gears.surface.load_silently(theme.wallpaperDir .. name .. ".png", nil)
                        tag_contexts[name] = surface
                        icon_png = theme.dir .. "/icons/" .. name .. ".png"
